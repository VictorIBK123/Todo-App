...
export default function EditViewAddTodo({route}){
    ...
    const textInputHandler =(value, type)=>{
        type=='title'?setTitle(value):setDetails(value)
    }
    const statusHandler=(value)=>{
        setCompleted(value)
    }
    const handleSave=(purpose)=>{
        if (!(title.length==0 || details.length==0 || priority==null)){
            if (purpose=='addTodo' && !canNowEdit){
                ...
                setCanNowEdit(true)
                setTodos([{todo: title, details, completed, priority, key: todos.length!=0?todos[0].key+1:1 ,date:date.toISOString()}, ...todos.filter((element)=>{
